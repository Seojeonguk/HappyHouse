$(function() {
	...
	$("#search-btn").click(function() {
		...
		if(category == "전체"|| category=="아파트") {
			apart_search(si,gu,dong,str);
		}
		if(category=="전체" || category=="연립다세대") {
			housing_search(si,gu,dong,str);
		}
	...
	if(domain_parameter!="") {
		...
		$("#si option").not("[name='si_init']").each(function(index,item) {
			if($(this).val() == decodeURIComponent(domain_parameter[1].split("=")[1])) {
				$(this).attr("selected","selected");
			}
		});
		load_gu_data();
		$("#gu option").not("[name='gu_init']").each(function(index,item) {
			if($(this).val() == decodeURIComponent(domain_parameter[2].split("=")[1])) {
				$(this).attr("selected","selected");
			}
		});
		load_dong_data();
		$("#dong option").not("[name='dong_init']").each(function(index,item) {
